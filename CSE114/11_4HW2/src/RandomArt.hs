...
exprToString (Rev e) = "-" ++ exprToString e
exprToString (ThreeFourths e1 e2 e3) = "("++exprToString e1 ++exprToString e2 ++ exprToString e3++")/4"
...
eval x y (Rev e) = -(eval x y e) 
eval x y (ThreeFourths e1 e2 e3) = (eval x y e1 + eval x y e2 + eval x y e3)/4
...
build d        
  ...
  |r==5 = Rev (build (d-1))
  |r==6 = ThreeFourths (build (d-1)) (build (d-1)) (build (d-1))
